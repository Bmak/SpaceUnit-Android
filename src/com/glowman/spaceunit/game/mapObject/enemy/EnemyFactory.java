...
public class EnemyFactory {
    ...
    /**
     * Black hole. Moving object with strong gravitation
     *
     * @return
     */
    private static Enemy createBlackHole() {
		Enemy result;

		Sprite image = new Sprite(Assets.joyPad);
		result = new Enemy(EnemyTypeENUM.BLACK_HOLE, image, false, BORDER_BEHAVIOUR.TELEPORT);

		BehaviourOptions options = new BlackHoleBehaviourOptions( _behaviourOptions.impactMaker);
		result.addBehaviour(new EnemyBlackHoleBehaviour(result, options));
		result.addBehaviour(new EnemyFollowBehaviour(result, _heroShip));
